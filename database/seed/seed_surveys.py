...
# import pymysql
from database.seed_data.survey_subcategory_data import survey_subcategory_data
...
# REVERSING UNIFICATION OF GOAL AND SURVEY CATEGORIES AND SUBCATEGORIES



# REVERSING UNIFICATION OF GOAL AND SURVEY CATS AND SUBCATS
...
def survey_category_seed():
    query = """
        INSERT INTO survey_categories
            (category_slug, name, created_at, updated_at)
        VALUES
...
def survey_subcategory_seed():
    ...
    for category_slug, subcategory_batch in batch_data.items():
        ...
        query = "INSERT INTO survey_subcategories (survey_category_id, subcategory_slug, name, created_at, updated_at) VALUES "
        query_values = []
    ...
    print(
        f"{sum(len(v) for v in survey_subcategory_data.values())} survey subcategories have been seeded."
    )
    return
...
def survey_question_seed(batched_data):
    ...
    for subcategory_id, questions in batched_data.items():
        query = """
            INSERT INTO survey_questions (survey_subcategory_id, question_slug, question_text, type, created_at, updated_at)
            VALUES %s
            ON DUPLICATE KEY UPDATE 
                survey_subcategory_id = VALUES(survey_subcategory_id),
        ...
        query_values = [
            (
                question["survey_subcategory_id"],
...
def prepare_survey_question_data():
    ...
    for subcategory in survey_question_data:
        ...
        query = "SELECT id FROM survey_subcategories WHERE subcategory_slug = %(subcategory_slug)s"
        ...
        for question in subcategory["questions"]:
            ...
            question_data = {
                "survey_subcategory_id": subcategory_id,
...
def _get_survey_question_id(subcategory_id, question_slug):
    query = """
        ...
        WHERE survey_subcategory_id = %(subcategory_id)s AND question_slug = %(question_slug)s;
    """
    try:
...
def _categorize_generic_question(question, survey_question_id, generic_answer_map_data):
    ...
    if question["type"] not in custom_answer_types and not re.match(r"^select-\d+$", question["type"]):
        query = """
...
def get_all_subcategory_ids():
    ...
    """
    query = "SELECT id, subcategory_slug FROM survey_subcategories"
    results = UserSurvey.db.query_db(query)
